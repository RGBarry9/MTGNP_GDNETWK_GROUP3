...
def cast_spell(game_server, message):
    ...
    if card.is_creature():
        ...
        # Grant priority to same player (they retain priority - RFC 0001 Section 8.1 rule 3)
        game_server._give_priority(player)
        return
...
def play_land(game_server, message):
    ...
    # ==========================================================
    # Step 6: Check active player + main phase (RFC 0001 SS7.5:
    # only the Active Player may play a land, and only in a Main Phase)
    # ==========================================================
    if game_server.game_state.active_player != player:
        game_server.send_error(
            game_server.connection_player.get(player_id),
            "WRONG_PHASE",
            "Only the active player can play a land"
        )
        return
...
def activate_ability(game_server, message):
    ...
    if effect.get("effect_type") == "ADD_MANA":
        ...
        # Grant priority to same player (RFC 0001 Section 8.1 rule 3)
        game_server._give_priority(player)
        return
