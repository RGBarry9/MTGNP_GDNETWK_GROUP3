...
class TurnManager:
    ...
    def next_phase(self) -> Phase:
        ...
        if index == len(self.PHASES) - 1:
            self.end_turn()
            # Actually begin the new active player's turn (Untap + Upkeep
            # run automatically inside start_turn(), landing on UPKEEP,
            # which is the first phase with a priority window). Without
            # this call, game_state.current_phase stayed stuck on
            # CLEANUP and no priority window ever reopened for the next
            # turn.
            self.start_turn()
