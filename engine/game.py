...
class Game:
    ...
    def start_game(self) -> None:
        ...
        # Initialize game state. RFC 0001 SS6.5 requires the server not
        # begin the first turn (IN_GAME) until both players have
        # completed mulligan, so we enter MULLIGAN here rather than
        # jumping straight to IN_GAME - that transition happens later,
        # once both players have kept a hand (see start_turn()).
        self.game_state.started = True
        self.game_state.game_over = False
        self.game_state.set_game_state(GameStateEnum.MULLIGAN)
    ...
    def start_turn(self) -> None:
        ...
        # Mulligan is over (this is only called once both players have
        # kept a hand) - the lifecycle now formally enters IN_GAME.
